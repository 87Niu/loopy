...
const reducer = (state = {
    ...
    switch(action.type) {
        case ACTIONS.ADD_DIGIT:
            ...
            if (action.digit === '.' && state.currentOperand === "") return {...state, currentOperand: "0."}
        ...
        case ACTIONS.CHOOST_OPERATION: 
            ...
            if (state.lastOperand === "") return {...state, lastOperand: state.currentOperand, operation: action.operation, currentOperand: ""}
            if (state.currentOperand === "") return {...state, operation: action.operation}
            return {...state, lastOperand: evaluate(state), operation: action.operation, currentOperand: ""}
        case ACTIONS.CLEAR: 
            return {...state, currentOperand: "", lastOperand: "", operation: ""}
        case ACTIONS.EVALUATE:
            if (state.currentOperand === "" || state.lastOperand === "" || state.operation === "") return state
            return {...state, currentOperand: evaluate(state), lastOperand: "", operation: ""}
        default:
            return state;
    }
}
